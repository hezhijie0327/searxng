...
class SXNGPlugin(Plugin):
    ...
    def _is_regex_pattern(self, text: str) -> bool:
        """Check if text is a regex pattern in the context of advanced syntax."""
        # 只有在特定上下文中的 /pattern/ 才被认为是正则表达式
        # 检查是否是高级语法中的正则表达式模式
        advanced_regex_patterns = [
            r'(?:^|\s)(?:\+|-)?(?:intitle|inurl|intext):/[^/]+/',  # intitle:/pattern/, -inurl:/pattern/ 等
            r'(?:^|\s)\+/[^/]+/',  # +/pattern/
            r'(?:^|\s)-/[^/]+/',  # -/pattern/
        ]

        return any(re.search(pattern, text, re.IGNORECASE) for pattern in advanced_regex_patterns)
    ...
    def _clean_query_for_engines(self, query: str) -> str:
        ...
        test_patterns = [
            ...
            r'(?:^|\s)-/',  # -/regex/ 格式
            # 移除了过于宽泛的 r'/.+/' 模式
            # 使用更精确的模式来检测高级语法中的正则表达式
            r'(?:intitle|inurl|intext):/[^/]+/',  # 只匹配高级语法中的正则表达式
            r'(?:^|\s)\+\w',  # +word 格式
        ]
    ...
    def _extract_remaining_terms(self, original_query: str, syntax: dict) -> list:
        ...
        for word in all_words:
            ...
            # 修复：使用更精确的正则表达式检测，避免误判文件路径
            # 只有在高级语法上下文中的 /pattern/ 才被跳过
            if self._is_regex_pattern(f" {word} "):  # 添加空格确保上下文检查
                continue
    ...
    def _is_advanced_syntax_word(self, word: str) -> bool:
        ...
        if word.startswith('-'):
            # 检查是否是 -word 格式（但不是文件路径）
            # 如果是 -/path/to/file 这种情况，不应该被认为是高级语法
            if len(word) > 1 and word[1] == '/':
                # 检查是否是 -/regex/ 格式（以 / 结尾）
                return word.endswith('/') and len(word) > 3
            # 其他 -word 格式
            return not any(word.startswith(prefix) for prefix in ['-site:', '-intitle:', '-inurl:', '-intext:'])
